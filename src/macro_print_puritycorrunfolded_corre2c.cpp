...
void macro_print_puritycorrunfolded_corre2c(int niter = 10)
{
    ...
    for(int bin = 0 ; bin < Nbin_jet_pt ; bin++)
    {
        ...
        for(int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++)
        {
            ...
            if(TMath::IsNaN(hcorr_data[bin]->Integral()))
            {
                std::cout<<"RL="<<R_L<<"     correction="<<purity*unfolding_weight*weight_pt/efficiency<<std::endl;
                std::cout<<"Eff="<<efficiency<<std::endl;
                std::cout<<"Pur="<<purity<<std::endl;
                std::cout<<"U  ="<<unfolding_weight<<std::endl;
                std::cout<<"W  ="<<weight_pt<<std::endl;
                break;
            }
        }
