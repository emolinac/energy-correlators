...
{
  // Failsafe
  if (syst_jer_array[0]==-999){std::cout<<"You need to set the beta values on the jer systematic."<<std::endl; return 0;}

  // Create output file
  TFile* fout = new TFile((output_folder+namef_ntuple_e2c_paircorrections_jer).c_str(),"RECREATE");
  ...
  for (int evt = 0 ; evt < mcrecotree->fChain->GetEntries() ; evt++)
  {
    ...
    // Set the new JES corrections
    Jet_4vector->SetPxPyPzE(mcrecotree->Jet_PX/1000./mcrecotree->Jet_JEC_Cor,
                            mcrecotree->Jet_PY/1000./mcrecotree->Jet_JEC_Cor,
                            mcrecotree->Jet_PZ/1000./mcrecotree->Jet_JEC_Cor,
