...
void macro_print_corre2c_mc_comp()
{
    ...
    for(int bin = 0 ; bin < Nbin_jet_pt ; bin++)
    {
        ...
        l_data[bin]->AddEntry(hcorr_data[bin],Form("%.1f<p^{jet}_{t}<%.1f GeV Data",jet_pt_binning[bin],jet_pt_binning[bin+1]),"lpf");
        l_data[bin]->AddEntry(hmc[bin]       ,Form("%.1f<p^{jet}_{t}<%.1f GeV MC",jet_pt_binning[bin],jet_pt_binning[bin+1]),"lpf");
