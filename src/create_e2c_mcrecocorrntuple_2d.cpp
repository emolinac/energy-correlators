...
int main()
{
        ...
        // Create output file
        TFile* fout = new TFile((output_folder + "ntuple_corrmcrecoe2c_2d.root").c_str(),"RECREATE");
        ...
        // Hadron corrections
        TH2F* hnum_pur    = new TH2F("hnum_pur"   , "", ic_p_nbins, ic_p_binning, sl_eta_nbins, sl_eta_binning);
        TH2F* hden_pur    = new TH2F("hden_pur"   , "", ic_p_nbins, ic_p_binning, sl_eta_nbins, sl_eta_binning);
        TH2F* hpurity     = new TH2F("hpurity"    , "", ic_p_nbins, ic_p_binning, sl_eta_nbins, sl_eta_binning);
        TH2F* hnum_eff    = new TH2F("hnum_eff"   , "", ic_p_nbins, ic_p_binning, sl_eta_nbins, sl_eta_binning);
        TH2F* hden_eff    = new TH2F("hden_eff"   , "", ic_p_nbins, ic_p_binning, sl_eta_nbins, sl_eta_binning);
        TH2F* hefficiency = new TH2F("hefficiency", "", ic_p_nbins, ic_p_binning, sl_eta_nbins, sl_eta_binning);
        ...
        ntuple_purity->Project("hnum_pur","h_eta:h_p",single_signal_cut);
        ntuple_purity->Project("hden_pur","h_eta:h_p");

        ntuple_efficiency_reco->Project("hnum_eff","h_eta_truth:h_p_truth",single_signal_cut);
        ntuple_efficiency_mc->Project("hden_eff","h_eta:h_p");
        ...
        std::cout<<"Working with MC Reco."<<std::endl;
        ...
        for (int evt = 0 ; evt < mcrecotree->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < mcrecotree->Jet_NDtr ; h1_index++) {
                        ...
                        double h1_purity     = hpurity->GetBinContent(hpurity->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                        double h1_efficiency = hefficiency->GetBinContent(hefficiency->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < mcrecotree->Jet_NDtr ; h2_index++) {
                                ...
                                double h2_purity     = hpurity->GetBinContent(hpurity->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                double h2_efficiency = hefficiency->GetBinContent(hefficiency->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                if (h2_purity > 1. || h2_efficiency > 1.) 
                                        continue;

                                double purity_correction = (h1_purity)*(h2_purity);

                                double h1_purity_err = hpurity->GetBinError(hpurity->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                                double h2_purity_err = hpurity->GetBinError(hpurity->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                ...
                                double h1_efficiency_err = hefficiency->GetBinError(hefficiency->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                                double h2_efficiency_err = hefficiency->GetBinError(hefficiency->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                double efficiency_error = sqrt(pow((h1_efficiency)*(h2_efficiency_err),2) + pow((h1_efficiency_err)*(h2_efficiency),2));

                                double nreco_ok_h1  = hnum_pur->GetBinContent(hnum_pur->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                                double nreco_h1     = hden_pur->GetBinContent(hden_pur->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                                double ntruth_ok_h1 = hnum_eff->GetBinContent(hnum_eff->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                                double ntruth_h1    = hden_eff->GetBinContent(hden_eff->FindBin(h1_4vector->P(),h1_4vector->Eta()));
                                double nreco_ok_h2  = hnum_pur->GetBinContent(hnum_pur->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                double nreco_h2     = hden_pur->GetBinContent(hden_pur->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                double ntruth_ok_h2 = hnum_eff->GetBinContent(hnum_eff->FindBin(h2_4vector->P(),h2_4vector->Eta()));
                                double ntruth_h2    = hden_eff->GetBinContent(hden_eff->FindBin(h2_4vector->P(),h2_4vector->Eta()));
