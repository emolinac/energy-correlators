...
int main()
{
        ...
        // Create output file
        TFile* fout = new TFile((output_folder + namef_ntuple_eec_paircorr_3d).c_str(),"RECREATE");
        ...
        for (int evt = 0 ; evt < datatree_2016->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < datatree_2016->Jet_NDtr ; h1_index++) {
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < datatree_2016->Jet_NDtr ; h2_index++) {
                                ...
                                double nreco_ok  = hnum_pur->GetBinContent(hnum_pur->FindBin(R_L, Jet_4vector->Pt(), h1_4vector->Pt()*h2_4vector->Pt()));
                                double nreco     = hden_pur->GetBinContent(hden_pur->FindBin(R_L, Jet_4vector->Pt(), h1_4vector->Pt()*h2_4vector->Pt()));
                                double ntruth_ok = hnum_eff->GetBinContent(hnum_eff->FindBin(R_L, Jet_4vector->Pt(), h1_4vector->Pt()*h2_4vector->Pt()));
                                double ntruth    = hden_eff->GetBinContent(hden_eff->FindBin(R_L, Jet_4vector->Pt(), h1_4vector->Pt()*h2_4vector->Pt()));
