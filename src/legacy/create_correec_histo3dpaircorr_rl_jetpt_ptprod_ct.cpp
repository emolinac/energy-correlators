...
{
        // Create output file
        TFile* fout = new TFile((output_folder + namef_3dpaircorr_rl_jetpt_ptprod_histos_ct).c_str(),"RECREATE");
        gROOT->cd();
        ...
        TH3F* h_eec_wmuon = new TH3F("h_eec_wmuon","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning, nbin_jet_pt_unfolding, unfolding_jet_pt_binning, nbin_ptprod, ptprod_binning);
        ...
        TH3F* h_npair_wmuon = new TH3F("h_npair_wmuon","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning, nbin_jet_pt_unfolding, unfolding_jet_pt_binning, nbin_ptprod, ptprod_binning);
        ...
        for (int evt = 0 ; evt < pseudodata->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < pseudodata->Jet_NDtr ; h1_index++) {
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < pseudodata->Jet_NDtr ; h2_index++) {
                                ...
                                h_eec_wmuon->Fill(h1_4vector->DeltaR(*h2_4vector), Jet_4vector->Pt(), h1_4vector->Pt() * h2_4vector->Pt(), momentum_weight*muon_weight);
                                h_npair_wmuon->Fill(h1_4vector->DeltaR(*h2_4vector), Jet_4vector->Pt(), h1_4vector->Pt() * h2_4vector->Pt(), muon_weight);
                        }
                }
