...
int main()
{
        ...
        for(int entry = 0 ; entry < ntuple_purity->GetEntries() ; entry++) {
                ...
                double coords[4] = {jet_pt_purity, R_L_purity, h1_p_purity, h2_p_purity};

                hden_pur->Fill(coords);

                if(abs(R_L_purity - R_L_truth_purity) < rl_resolution)
                        hnum_pur->Fill(coords);
        }
        ...
        for (int evt = 0 ; evt < datatree_2016->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < datatree_2016->Jet_NDtr ; h1_index++) {
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < datatree_2016->Jet_NDtr ; h2_index++) {
                                ...
                                double efficiency       = hefficiency->GetBinContent(globalbin);
                                double purity_error     = hpurity->GetBinError(globalbin);        
                                double efficiency_error = hefficiency->GetBinError(globalbin);

                                double nreco_ok  = 0;//hnum_pur->GetBinContent(hnum_pur->FindBin(R_L, h1_4vector->Pt(), h2_4vector->Pt()));
                                double nreco     = 0;//hden_pur->GetBinContent(hden_pur->FindBin(R_L, h1_4vector->Pt(), h2_4vector->Pt()));
                                double ntruth_ok = 0;//hnum_eff->GetBinContent(hnum_eff->FindBin(R_L, h1_4vector->Pt(), h2_4vector->Pt()));
                                double ntruth    = 0;//hden_eff->GetBinContent(hden_eff->FindBin(R_L, h1_4vector->Pt(), h2_4vector->Pt()));
