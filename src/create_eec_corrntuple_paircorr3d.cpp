...
int main()
{
        ...
        TH3F* hnum_pur    = new TH3F("hnum_pur"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_jet_pt_corrections, jet_pt_corrections_binning, nbin_pt_product, pt_product_binning);
        TH3F* hden_pur    = new TH3F("hden_pur"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_jet_pt_corrections, jet_pt_corrections_binning, nbin_pt_product, pt_product_binning);
        TH3F* hpurity     = new TH3F("hpurity"    , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_jet_pt_corrections, jet_pt_corrections_binning, nbin_pt_product, pt_product_binning);
        TH3F* hnum_eff    = new TH3F("hnum_eff"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_jet_pt_corrections, jet_pt_corrections_binning, nbin_pt_product, pt_product_binning);
        TH3F* hden_eff    = new TH3F("hden_eff"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_jet_pt_corrections, jet_pt_corrections_binning, nbin_pt_product, pt_product_binning);
        TH3F* hefficiency = new TH3F("hefficiency", "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_jet_pt_corrections, jet_pt_corrections_binning, nbin_pt_product, pt_product_binning);
        ...
        // DELETE LATER
        TH2F* hnum_pur_2d    = new TH2F("hnum_pur_2d"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_pt_product, pt_product_binning);
        TH2F* hden_pur_2d    = new TH2F("hden_pur_2d"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_pt_product, pt_product_binning);
        TH2F* hpurity_2d     = new TH2F("hpurity_2d"    , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_pt_product, pt_product_binning);
        TH2F* hnum_eff_2d    = new TH2F("hnum_eff_2d"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_pt_product, pt_product_binning);
        TH2F* hden_eff_2d    = new TH2F("hden_eff_2d"   , "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_pt_product, pt_product_binning);
        TH2F* hefficiency_2d = new TH2F("hefficiency_2d", "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning, nbin_pt_product, pt_product_binning);
        ...
        ntuple_purity->Project("hnum_pur_2d", "h1_pt*h2_pt:R_L", pair_matching_cut);
        ntuple_purity->Project("hden_pur_2d", "h1_pt*h2_pt:R_L");
        ntuple_efficiency_reco->Project("hnum_eff_2d", "h1_pt_truth*h2_pt_truth:R_L_truth", pair_matching_cut);
        ntuple_efficiency_mc->Project("hden_eff_2d", "h1_pt*h2_pt:R_L");

        hpurity_2d->Divide(hnum_pur_2d, hden_pur_2d, 1, 1, "B");
        hefficiency_2d->Divide(hnum_eff_2d, hden_eff_2d, 1, 1, "B");

        hpurity_2d->Smooth();
        hefficiency_2d->Smooth();
        ...
        hpurity_2d->Draw("col text");
        hpurity_2d->SetTitle("Purity Correction;R_{L};p_{T,i}*p_{T,j}(GeV^{2})");
        hpurity_2d->GetXaxis()->SetRangeUser(rl_nominal_binning[0], rl_nominal_binning[nbin_rl_nominal]);
        hpurity_2d->GetYaxis()->SetRangeUser(pt_product_binning[0], pt_product_binning[nbin_pt_product]);
        gPad->SetLogx(1);
        gPad->SetLogy(1);
        c->Print("../src-analysis/plots/pair_purity_correction_ptproduct.pdf");

        hefficiency_2d->Draw("col text");
        hefficiency_2d->SetTitle("Efficiency Correction;R_{L};p_{T,i}*p_{T,j}(GeV^{2})");
        hefficiency_2d->GetXaxis()->SetRangeUser(rl_nominal_binning[0], rl_nominal_binning[nbin_rl_nominal]);
        hefficiency_2d->GetYaxis()->SetRangeUser(pt_product_binning[0], pt_product_binning[nbin_pt_product]);
        gPad->SetLogx(1);
        gPad->SetLogy(1);
        c->Print("../src-analysis/plots/pair_efficiency_correction_ptproduct.pdf");
        ...
        for (int evt = 0 ; evt < datatree_2016->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < datatree_2016->Jet_NDtr ; h1_index++) {
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < datatree_2016->Jet_NDtr ; h2_index++) {
                                ...
                                double purity           = hpurity->GetBinContent(hpurity->FindBin(R_L, Jet_4vector->Pt(),h1_4vector->Pt()*h2_4vector->Pt()));        
                                double efficiency       = hefficiency->GetBinContent(hefficiency->FindBin(R_L, Jet_4vector->Pt(),h1_4vector->Pt()*h2_4vector->Pt()));
                                double purity_error     = hpurity->GetBinError(hpurity->FindBin(R_L, Jet_4vector->Pt(),h1_4vector->Pt()*h2_4vector->Pt()));        
                                double efficiency_error = hefficiency->GetBinError(hefficiency->FindBin(R_L, Jet_4vector->Pt(),h1_4vector->Pt()*h2_4vector->Pt()));
