...
{
        // Create output file
        TFile* fout = new TFile((output_folder + namef_3dpaircorr_rl_jetpt_weightpt_histos_ct).c_str(),"RECREATE");
        gROOT->cd();
        ...
        TFile* fefficiency_muon_2017_trg = new TFile((muons_folder + "TRGEff_MC_2016.root").c_str());
        ...
        TH2D* h2_muon_2017_trkeff_data = (TH2D*) fefficiency_muon_2017_trk->Get("Hist_ALL_2016_ETA_PT_Eff");
        TH2D* h2_muon_2017_trgeff_data = (TH2D*) fefficiency_muon_2017_trg->Get("Hist_ALL_2016_ETA_PT_Eff");
        ...
        for (int evt = 0 ; evt < pseudodata->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < pseudodata->Jet_NDtr ; h1_index++) {
                        if (abs(pseudodata->Jet_Dtr_ID[h1_index]) < 100)
                                continue;
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < pseudodata->Jet_NDtr ; h2_index++) {
                                if (abs(pseudodata->Jet_Dtr_ID[h2_index]) < 100)
                                        continue;
        ...
        for (int evt = 0 ; evt < truthdata->fChain->GetEntries() ; evt++) {
                ...
                for (int h1_index = 0 ; h1_index < truthdata->MCJet_Dtr_nmcdtrs ; h1_index++) {
                        if (abs(truthdata->MCJet_Dtr_ID[h1_index]) < 100)
                                continue; 
                        ...
                        for (int h2_index = h1_index+1 ; h2_index < truthdata->MCJet_Dtr_nmcdtrs ; h2_index++) {
                                if (abs(truthdata->MCJet_Dtr_ID[h2_index]) < 100)
                                        continue; 
