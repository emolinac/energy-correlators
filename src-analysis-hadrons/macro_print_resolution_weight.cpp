...
void macro_print_resolution_weight()
{
        ...
        TH1F* hres   = new TH1F("hres","",1000,-.4,.4);
        TH1F* hratio = new TH1F("hratio","",1000,0.,3);
        hres->Sumw2();
        hratio->Sumw2();
        ...
        ntuple_dtrmatch->Project("hres"  ,"weight_pt-weight_pt_truth",pair_cut+"weight_pt!=0&&weight_pt_truth!=-999");
        ntuple_dtrmatch->Project("hratio","weight_pt/weight_pt_truth",pair_cut+"weight_pt!=0&&weight_pt_truth!=-999");
        ...
        for (int bin = 1 ; bin <= hres->GetNbinsX()/2. ; bin++) {
                ...
                if (integral/total < 0.95) {
                        std::cout<<"Between "<<hres->GetBinCenter(bin)<<" and "<<hres->GetBinCenter(hres->GetNbinsX()-bin)<<" is the 95%% of the sample"<<std::endl; 
        ...
        for (int bin = 1 ; bin <= hres->GetNbinsX()/2 ; bin++) {
                ...
                if (integral/total < 0.68) {
                        std::cout<<"Between "<<hres->GetBinCenter(bin)<<" and "<<hres->GetBinCenter(hres->GetNbinsX()-bin)<<" is the 68%% of the sample"<<std::endl; 
        ...
        for (int bin = 1 ; bin <= hratio->GetNbinsX()/2. ; bin++) {
                ...
                if (integral/total < 0.95) {
                        std::cout<<"Between "<<hratio->GetBinCenter(bin)<<" and "<<hratio->GetBinCenter(hratio->GetNbinsX()-bin)<<" is the 95%% of the sample"<<std::endl; 
        ...
        hres->Draw();
        hres->SetTitle(";#Delta weight(Reco,Truth);");
        ...
        c->Print("./plots/resolution_weight.pdf");

        hratio->Draw();
        hratio->SetTitle(";weight(Reco)/weight(Truth);");
        ...
        // gPad->SetLogy(0);
        c->Print("./plots/resolution_weight_ratio.pdf");
}