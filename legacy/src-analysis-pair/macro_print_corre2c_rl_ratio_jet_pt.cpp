...
void macro_print_correec_rl_ratio_jet_pt()
{
    ...
    for (int jet_pt_bin = 0 ; jet_pt_bin < nbin_jet_pt ; jet_pt_bin++)
    {
        ...
        hmc[jet_pt_bin] = new TH1F(Form("hmc[%i]",jet_pt_bin),"",nbin_rl_nominal,rl_min, rl_max);
        ...
        ntuple_mc->Project(Form("hmc[%i]",jet_pt_bin),"R_L", eec_jet_pt_cut[jet_pt_bin]);
    ...
    s_data->SetTitle(Form("#Delta R_{L}(truth-reco)<%.3f;R_{L};Norm. eec (Meas/MC)",rl_resolution));
    l_data->Draw("SAME");

    tex->DrawLatexNDC(0.25,0.25,"LHCb Internal");
    
    c->Print(Form("./plots/corr_eec_ratio_jet_pt_deltarleq%.3f.pdf",rl_resolution));
}