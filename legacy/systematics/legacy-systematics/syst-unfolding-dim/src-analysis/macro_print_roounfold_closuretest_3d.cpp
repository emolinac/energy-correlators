...
void macro_print_roounfold_closuretest_3d(int niter = nominal_niter)
{
    ...
    TH3D* htrue_ref = new TH3D("htrue_ref","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning,nbin_weight_unfolding,weight_unfoldingbinning);
    TH3D* hct       = new TH3D("hct"      ,"",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning,nbin_weight_unfolding,weight_unfoldingbinning);
    ...
    for (int evt = 0 ; evt < ntuple->GetEntries() ; evt++)
    {
        ...
        {
            htrue_ref->Fill(R_L_truth,jet_pt_truth,weight_pt_truth);
            continue;
        }

        response->Fill(R_L, jet_pt, weight_pt, R_L_truth, jet_pt_truth, weight_pt_truth);
    ...
    TH2D* htrue_ref_rl_jet_pt      = new TH2D("htrue_ref_rl_jet_pt"      ,"",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning);
    TH2D* hunfolded_bayes_rl_jet_pt= new TH2D("hunfolded_bayes_rl_jet_pt","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning);
    ...
    htrue_ref_rl_jet_pt        = (TH2D*) htrue_ref->Project3D("yx");
    ...
    hunfolded_bayes_rl_jet_pt  = (TH2D*) hunfolded_bayes->Project3D("yx");
    
    hct_rl->Divide(htrue_ref_rl,hunfolded_bayes_rl,1,1);
    hct_jet_pt->Divide(htrue_ref_jet_pt,hunfolded_bayes_jet_pt,1,1);
    
    hct_rl_jet_pt->Divide(htrue_ref_rl_jet_pt,hunfolded_bayes_rl_jet_pt,1,1);
    ...
    gPad->SetLogx(1);
    lrl->Draw("SAME");
    c->Print(Form("./plots/unfolded3d_closuretest_rl_niter%i.pdf",Niter));    
    gPad->SetLogx(0);
    ...
    ljet_pt->Draw("SAME");
    c->Print(Form("./plots/unfolded3d_closuretest_jet_pt_niter%i.pdf",Niter));    
    ...
    c2d->Print(Form("./plots/unfolded3d_closuretest_niter%i.pdf",Niter));    
}