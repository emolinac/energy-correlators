...
void macro_print_fullcorrchargedeec_paircorr_2dunf(int niter = nominal_niter, bool do_print = true)
{
    ...
    TFile* fout_linear = new TFile((output_folder + namef_histos_paircorr_eec).c_str(),"RECREATE");
    gROOT->cd();
    ...
    for (int bin = 0 ; bin < nbin_jet_pt ; bin++)
    {
        ...
        hcorr_eec_eqcharge[bin]  = new TH1F(Form("hcorr_eec_eqcharge%i",bin) ,"", nbin_rl_nominal,rl_nominal_binning );
        hcorr_eec_neqcharge[bin] = new TH1F(Form("hcorr_eec_neqcharge%i",bin),"", nbin_rl_nominal,rl_nominal_binning );
        ...
        for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++)
        {
            ...
            if (eq_charge>0)  hcorr_eec_eqcharge[bin]->Fill(R_L,purity*unfolding_weight*weight_pt/efficiency);
            else if (eq_charge<0) hcorr_eec_neqcharge[bin]->Fill(R_L,purity*unfolding_weight*weight_pt/efficiency);
        }
    ...
    if (do_print) c->Print(Form("./plots/paircorreec_niter%i_linbinning_2dunf.pdf",niter));
    ...
    for (int bin = 0 ; bin < nbin_jet_pt ; bin++)
    {
        s_data->Add(hcorr_eec_eqcharge[bin] ,"E1 X0 ");
        s_data->Add(hcorr_eec_neqcharge[bin],"E1 X0 ");
    ...
    if (do_print) c->Print(Form("./plots/paircorrchargedeec_niter%i_2dunf.pdf",niter));
    ...
    if (do_print) c->Print(Form("./plots/paircorreec_niter%i_2dunf.pdf",niter));
}
