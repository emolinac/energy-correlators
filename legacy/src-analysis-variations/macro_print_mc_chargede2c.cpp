...
void macro_print_mc_chargede2c()
{
    ...
    for (int bin = 0 ; bin < nbin_jet_pt ; bin++)
    {
        ...
        hmcreco_pp[bin] = new TH1F(Form("hmcreco_pp[%i]",bin)  ,"",nbin_rl_nominal,rl_binning);
        hmcreco_pm[bin] = new TH1F(Form("hmcreco_pm[%i]",bin)  ,"",nbin_rl_nominal,rl_binning);
        hmcreco_mm[bin] = new TH1F(Form("hmcreco_mm[%i]",bin)  ,"",nbin_rl_nominal,rl_binning);
        hmcreco_all[bin] = new TH1F(Form("hmcreco_all[%i]",bin),"",nbin_rl_nominal,rl_binning);
        
        hmc_pp[bin] = new TH1F(Form("hmc_pp[%i]" ,bin)  ,"",nbin_rl_nominal,rl_binning);
        hmc_pm[bin] = new TH1F(Form("hmc_pm[%i]" ,bin)  ,"",nbin_rl_nominal,rl_binning);
        hmc_mm[bin] = new TH1F(Form("hmc_mm[%i]" ,bin)  ,"",nbin_rl_nominal,rl_binning);
