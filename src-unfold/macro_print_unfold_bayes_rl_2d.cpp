...
void macro_print_unfold_bayes_rl_2d(int Niter = 1, double jet_pt_min_local = 20, double jet_pt_max_local = 100)
{
    ...
    TH2F* hmeas = new TH2F("hmeas","",Nbin_R_L+2,em_rlunfolding_binning,Nbin_jet_pt,jet_pt_binning);
    TH2F* htrue = new TH2F("htrue","",Nbin_R_L+2,em_rlunfolding_binning,Nbin_jet_pt,jet_pt_binning);
    ...
    for(int evt = 0 ; evt < ntuple->GetEntries() ; evt++)
    {
        ...
        ntuple->GetEntry(evt);
        if(R_L_truth!=-999) response->Fill(R_L, jet_pt, R_L_truth, jet_pt_truth);
    }
    ...
    auto* hunfolded_bayes = (TH2*) unfold.Hunfold();
    ...
    // c->Print(Form("../plots/unfolding/unfolded_rl_bayes_iter%i_2d.pdf",Niter));    
}