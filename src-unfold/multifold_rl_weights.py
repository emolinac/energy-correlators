...
theta0_G_jp_np = np.array(root_tree["weight_truth"])
theta_unknown_S_rl_np = np.array(root_tree_data["R_L"])
theta_unknown_S_jp_np = np.array(root_tree_data["weight"])
...
theta0_S = np.column_stack((root_tree["R_L"],root_tree["weight"]))
...
_,_,_ = ax22.hist(theta0_G[:,1]       ,bins=np.linspace(0,0.2,25),color='blue',histtype="step",label="MC, true",lw="2")
_,_,_ = ax22.hist(theta0_S[:,1]       ,bins=np.linspace(0,0.2,25),color='green',histtype="step",label="MC, reco",lw="2")
_,_,_ = ax22.hist(theta_unknown_S[:,1],bins=np.linspace(0,0.2,25),color='orange',histtype="step",label="Data, reco",lw="2")
...
plt.savefig("./plots/{}-iterations_{}-hlayers_{}-nodes_rl_weight.pdf".format(niterations, 3, nnodes), format="pdf", bbox_inches="tight")
plt.show()
# for ax in (ax21, ax22):
#     for i in range(2):
