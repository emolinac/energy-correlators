...
for iteration in  range(1, niterations):
    ...
    myweights = of.omnifold(theta0_G_train, theta0_S_train, theta_unknown_S_train, iteration, model, 0,nepochs, nbatch_size)
    ...
    h_weight_validate,bins_weight_validate,_ = ax33test.hist(theta0_G_validate[:,2],bins=np.linspace(0,0.05,10)    ,color='orange',histtype="step",label="Data, reco",lw="2")

    h_rl_unfolded    ,bins_rl_unfolded    ,_ = ax11test.hist(theta0_G_train[:,0], weights=myweights[iteration-1, 1, :], bins=np.linspace(0.00999,0.5,15),color='black',histtype="step",label="Unfolded",lw="2")
    h_jetpt_unfolded ,bins_jetpt_unfolded ,_ = ax22test.hist(theta0_G_train[:,1], weights=myweights[iteration-1, 1, :], bins=[20,30,50,100]     ,color='black',histtype="step",label="Unfolded",lw="2")
    h_weight_unfolded,bins_weight_unfolded,_ = ax33test.hist(theta0_G_train[:,2], weights=myweights[iteration-1, 1, :], bins=np.linspace(0,0.05,10)    ,color='black',histtype="step",label="Unfolded",lw="2")
    ...
    ax33.set(xlabel="$weight$",ylabel="Unfolded/Truth")
...
# plt.savefig("./plots/closuretest_{}iterations_{}hlayers_{}nodes_{}epochs_{}nbatchsize.pdf".format(niterations, 3, nnodes, nepochs, nbatch_size), format="pdf", bbox_inches="tight")
plt.show()
