...
void macro_print_fullcorreec_paircorr_2dunf_h_pt_corr(int niter = nominal_niter, bool do_print = true, bool do_jet_unfolding = false, bool apply_alice_factor = false)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + namef_histos_paircorr_eec_h1h2).c_str(),"RECREATE");
        gROOT->cd();

        TFile* fcorr = new TFile((output_folder + namef_ntuple_eec_paircorr_h1h2).c_str()); 
        if (fcorr->IsZombie()) 
                return;
        ...
        if (do_print) 
                c->Print(Form("./plots/unfolded2d_unf-niter%i_ratio_h1h2corr.pdf",niter));
        ...
        if (do_print) 
                c->Print(Form("./plots/unfolded1d_jetpt_unf-niter%i_ratio_h1h2corr.pdf",niter));

        hresponse_jet->Draw("col text");
        gPad->SetLogx(1);
        gPad->SetLogy(1);
        if (do_print) 
                c->Print("./plots/jet_pt_response_matrix_h1h2corr.pdf");
        ...
        if (do_print) 
                c->Print(Form("./plots/corrtau_unf-niter%i_jetptunf-%s_2dunf_h1h2corr.pdf",niter,(do_jet_unfolding)?"yes":"no"));
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                if (do_print)
                        c->Print(Form("./plots/corrchargedeec_jetptbin%i_unf-niter%i_jetptunf-%s_2dunf_h1h2corr.pdf", bin, niter,(do_jet_unfolding)?"yes":"no"));
        }
        ...
        if (do_print) 
                c->Print(Form("./plots/correec_unf-niter%i_jetptunf-%s_2dunf_h1h2corr.pdf",niter,(do_jet_unfolding)?"yes":"no"));
}
