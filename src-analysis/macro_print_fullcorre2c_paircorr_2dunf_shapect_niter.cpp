...
void macro_print_fullcorreec_paircorr_2dunf_shapect_niter(int niter = nominal_niter, int ct_niter = 1, bool do_print = true, bool compare_to_truth = true)
{
        ...
        for (int ct_iter = 0 ; ct_iter < ct_niter ; ct_iter++) {
                ...
                for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                        ...
                        for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++) {
                                ...
                                hcorr_eec_nonorm[bin]->Fill(R_L,purity*unfolding_weight*weight_pt*content_shift/efficiency);
                                hcorr_eec[bin]->Fill(R_L,purity*unfolding_weight*weight_pt*content_shift/efficiency);
                                hcorr_eec_nounf[bin]->Fill(R_L,purity*weight_pt*content_shift/efficiency);
        ...
        if (compare_to_truth) {
                ...
                for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {   
                        ...
                        fout_dev->cd();
                        hcorr_ratio_eec_total[bin]->Write(Form("h_deviations%i",bin));
