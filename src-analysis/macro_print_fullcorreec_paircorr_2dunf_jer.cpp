...
void macro_print_fullcorreec_paircorr_2dunf_jer(int niter = nominal_niter, bool do_print = true, bool compare_to_nominal = false)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + namef_histos_paircorr_eec_jer).c_str(),"RECREATE");
        gROOT->cd();

        TFile* fcorr = new TFile((output_folder + namef_ntuple_eec_paircorr_jer).c_str()); 
        if (fcorr->IsZombie()) 
                return;
        ...
        TFile* f = new TFile((output_folder + namef_ntuple_eec_paircorrections_jer).c_str());
        ...
        if (do_print) 
                c->Print(Form("./plots/unfolded2d_unf-niter%i_ratio_jer.pdf",niter));
        ...
        if (do_print) 
                c->Print(Form("./plots/corrtau_unf-niter%i_2dunf_jer.pdf",niter));
        ...
        if (do_print) 
                c->Print(Form("./plots/correec_unf-niter%i_2dunf_jer.pdf",niter));
        ...
        if (compare_to_nominal) {
                ...
                gPad->SetLogx(1);
                gPad->SetLogy(1);
                if (do_print) c->Print(Form("./plots/nominal_jer_comp_unf-niter%i_ratio.pdf",niter));
        }
}
