...
void macro_print_fullcorreec_paircorr_2dunf_5dcorr(int niter = nominal_niter, bool do_print = true, bool do_jet_unfolding = false, bool apply_alice_factor = false)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + namef_histos_paircorr_eec_5d).c_str(),"RECREATE");
        gROOT->cd();

        TFile* fcorr = new TFile((output_folder + namef_ntuple_eec_paircorr_5d).c_str()); 
        if (fcorr->IsZombie()) 
                return;
        ...
        ntuple_data->Project("hpuritycorrected" , "jet_pt:R_L","purity*(purity>0&&purity<1)");
        ntuple_data->Project("hpuritycorrected_ref", "jet_pt:R_L","purity*(purity>0&&purity<1)");
        ...
        if (do_print) 
                c->Print(Form("./plots/unfolded2d_unf-niter%i_ratio_5dcorrwdoublecount.pdf",niter));
        ...
        // if (do_print) 
        //         c->Print(Form("./plots/unfolded1d_jetpt_unf-niter%i_ratio_5dcorrwdoublecount.pdf",niter));

        // hresponse_jet->Draw("col text");
        // gPad->SetLogx(1);
        // gPad->SetLogy(1);
        // if (do_print) 
        //         c->Print("./plots/jet_pt_response_matrix_5dcorrwdoublecount.pdf");
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++) {
                        ...
                        if (purity <= 0 || efficiency <= 0)
                                continue;
                        
                        // if (purity_relerror > .5 || efficiency_relerror > .6)
                        //         continue;
        ...
        if (do_print) 
                c->Print(Form("./plots/corrtau_unf-niter%i_jetptunf-%s_2dunf_5dcorrwdoublecount.pdf",niter,(do_jet_unfolding)?"yes":"no"));
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                if (do_print)
                        c->Print(Form("./plots/corrchargedeec_jetptbin%i_unf-niter%i_jetptunf-%s_2dunf_5dcorrwdoublecount.pdf", bin, niter,(do_jet_unfolding)?"yes":"no"));
        }
        ...
        if (do_print) 
                c->Print(Form("./plots/correec_unf-niter%i_jetptunf-%s_2dunf_5dcorrwdoublecount.pdf",niter,(do_jet_unfolding)?"yes":"no"));
}
