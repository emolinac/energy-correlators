...
void macro_print_fullcorreec_paircorr_3dunf_ct_niter_wweights(int niter = nominal_niter, int ct_niter = 10, bool do_print = true, bool compare_to_truth = true)
{
        ...
        for (int ct_iter = 0 ; ct_iter < ct_niter ; ct_iter++) {
                ...
                ntuple_pseudodata->Project("hpuritycorrected"    ,"h1_pt*h2_pt:jet_pt:R_L","purity*weight_pt");
                ntuple_pseudodata->Project("hpuritycorrected_ref","h1_pt*h2_pt:jet_pt:R_L","purity*weight_pt");
                ...
                if (do_print) 
                        c->Print(Form("./plots/unfolded3d_3dunf-niter%i_ratio_wweights_ct.pdf",niter));
        ...
        if (compare_to_truth) {
                ...
                gPad->SetLogy(1);
                if (do_print) 
                        c->Print(Form("./plots/closuretest_3dunf-niter%i_ratio_ctniter%i_wweights.pdf",niter,ct_niter));
                ...
                gPad->SetLogy(1);
                if (do_print) 
                        c->Print(Form("./plots/closuretest_3dunf-niter%i_ratio_ctniter%i_npairs_wweights.pdf",niter,ct_niter));
        }
}
