...
void macro_print_fullcorreec_paircorr_2dunf_3dcorr(int niter = nominal_niter, bool do_print = true, bool do_jet_unfolding = false, bool apply_alice_factor = false)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + namef_histos_paircorr_eec_3d).c_str(),"RECREATE");
        gROOT->cd();
        ...
        // if (do_print) 
        //         c->Print(Form("./plots/unfolded1d_jetpt_unf-niter%i_ratio_3dcorr.pdf",niter));

        // hresponse_jet->Draw("col text");
        // gPad->SetLogx(1);
        // gPad->SetLogy(1);
        // if (do_print) 
        //         c->Print("./plots/jet_pt_response_matrix_3dcorr.pdf");
        ...
        if (do_print) 
                c->Print(Form("./plots/corrtau_unf-niter%i_jetptunf-%s_2dunf_3dcorr.pdf",niter,(do_jet_unfolding)?"yes":"no"));
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                if (do_print)
                        c->Print(Form("./plots/corrchargedeec_jetptbin%i_unf-niter%i_jetptunf-%s_2dunf_3dcorr.pdf", bin, niter,(do_jet_unfolding)?"yes":"no"));
        }
        ...
        if (do_print) 
                c->Print(Form("./plots/correec_unf-niter%i_jetptunf-%s_2dunf_3dcorr.pdf",niter,(do_jet_unfolding)?"yes":"no"));
}
