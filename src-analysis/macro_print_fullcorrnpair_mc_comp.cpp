...
void macro_print_fullcorrnpair_mc_comp(int niter = 15, bool do_print = true)
{
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++) {
                        ...
                        hcorr_eec[bin]->Fill(R_L,purity*unfolding_weight/efficiency);

                        hcorr_eec_syst[bin]->Fill(R_L,purity*unfolding_weight/efficiency);
                        hcorr_eec_nounf[bin]->Fill(R_L,purity/efficiency);
