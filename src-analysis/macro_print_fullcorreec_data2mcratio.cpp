...
void macro_print_fullcorreec_data2mcratio(int niter = 15, bool do_print = true)
{
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++) {
                        ...
                        hcorr_eec[bin]->Fill(R_L,weight_pt*weight*purity/efficiency);
                }
        ...
        TH1F* hdata2mc_ratio[nbin_jet_pt]; 
        ...
        for(int bin = 0 ; bin < nbin_jet_pt ; bin ++) {
                ...
                hdata2mc_ratio[bin] = new TH1F(Form("hdata2mc_ratio%i",bin),"", nbin_rl_nominal, rl_nominal_binning);

                set_histogram_style(hdata2mc_ratio[bin], corr_marker_color_jet_pt[bin], std_line_width, corr_marker_style_jet_pt[bin], std_marker_size);
                
                hdata2mc_ratio[bin]->Divide(hcorr_eec[bin],hmc[bin],1,1);

                s_data[bin]->Add(hdata2mc_ratio[bin],"E");
                
                s_data[bin]->Draw("NOSTACK");
                
                s_data[bin]->SetTitle(Form("%.1f<p^{jet}_{t}(GeV)<%.1f;R_{L};data/truth", jet_pt_binning[bin], jet_pt_binning[bin+1]));

                s_data[bin]->SetMaximum(2);
                s_data[bin]->SetMinimum(0.5);
                
                l_data[bin]->AddEntry(hdata2mc_ratio[bin], "data/mc", "p");
        ...
        c->Print("./plots/data2mcratio_eec_fullsim.pdf");
}