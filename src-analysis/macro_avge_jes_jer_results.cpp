...
void macro_avge_jes_jer_results(int niter = 4, int niter_jet = 4)
{
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                hcorr_eec_avge[bin]->Scale(1./(double) nvariations);
                hcorr_tau_avge[bin]->Scale(1./(double) nvariations);
                hcorr_eec_eqcharge_avge[bin]->Scale(1./(double) nvariations);
                hcorr_eec_neqcharge_avge[bin]->Scale(1./(double) nvariations);
