...
void macro_print_histocorreec_nojetmatch_rl_jetpt_weightpt_ct(int niter = 3)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + Form("histos_eec_3dcorr_rl_jetpt_weightpt_niter%i_nojetmatch_ct.root",niter)).c_str(),"RECREATE");
        gROOT->cd();

        TFile* fcorr = new TFile((output_folder + namef_3dpaircorr_nojetmatch_rl_jetpt_weightpt_histos_ct).c_str());
        TFile* fdata = new TFile((output_folder + namef_3dpaircorr_rl_jetpt_weightpt_histos).c_str()); 
        ...
        // // add a smoothed version of the muon efficiency
        // h_muon_eff->Smooth();
        ...
        for (int evt = 0 ; evt < ntuple->GetEntries() ; evt++) {
                ...
                if (R_L_truth != -999) {
                        response->Fill(R_L_reco, jet_pt_reco, weight_pt_reco, R_L_truth, jet_pt_truth, weight_pt_truth, weight_pt_truth);
                        response_npair->Fill(R_L_reco, jet_pt_reco, weight_pt_reco, R_L_truth, jet_pt_truth, weight_pt_truth);
                }
        }

        TH3F* h_eec_purity_corrected = new TH3F("h_eec_purity_corrected","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning, nbin_weight, weight_binning);
