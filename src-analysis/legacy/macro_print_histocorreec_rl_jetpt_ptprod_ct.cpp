...
void macro_print_histocorreec_rl_jetpt_ptprod_ct(int niter = 3, int niter_jet = 15)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + Form("histos_eec_3dcorr_rl_jetpt_ptprod_niter%i_niterjet%i_ct.root",niter,niter_jet)).c_str(),"RECREATE");
        gROOT->cd();

        TFile* fcorr = new TFile((output_folder + namef_3dpaircorr_rl_jetpt_ptprod_histos_ct).c_str());
        ...
        TH3F* h_eec_wmuon   = (TH3F*) fcorr->Get("h_eec_wmuon");
        ...
        TFile* fjet = new TFile((output_folder + namef_ntuple_jet_purity_ct).c_str());
        ...
        for (int i = 1 ; i <= h_eec_unfolded->GetNbinsX(); i++) {
                for (int j = 1 ; j <= h_eec_unfolded->GetNbinsY(); j++) {
                        for (int k = 1 ; k <= h_eec_unfolded->GetNbinsZ() ; k++) {
                                // double reweight = h_purity_jet->GetBinContent(j)/h_muon_eff->GetBinContent(j)/h_efficiency_jet->GetBinContent(j);
        ...
        TH3F* h_eec_muoneffcorr = new TH3F("h_eec_muoneffcorr","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning, nbin_ptprod, ptprod_binning);
        h_eec_muoneffcorr->Divide(h_eec_wmuon,h_eec);

        h_eec_unfolded->Multiply(h_eec_muoneffcorr);
        ...
        TH3F* h_npair_muoneffcorr = new TH3F("h_npair_muoneffcorr","",nbin_rl_nominal_unfolding,unfolding_rl_nominal_binning,nbin_jet_pt_unfolding,unfolding_jet_pt_binning, nbin_ptprod, ptprod_binning);
        h_npair_muoneffcorr->Divide(h_npair_wmuon,h_npair);

        h_npair_unfolded->Multiply(h_npair_muoneffcorr);
