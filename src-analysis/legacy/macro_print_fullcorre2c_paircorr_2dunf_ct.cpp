...
void macro_print_fullcorreec_paircorr_2dunf_ct(int niter = nominal_niter, bool do_print = true, bool compare_to_nominal = false, bool compare_to_truth = true)
{
    // Open the necessary files
    TFile* fout        = new TFile((output_folder + namef_histos_paircorr_eec_ct).c_str(),"RECREATE");
    TFile* fout_linear = new TFile((output_folder + namef_histos_paircorr_eec_ct).c_str(),"RECREATE");
    gROOT->cd();
    ...
    TH1F* hcorr_eec_nounf_l[nbin_jet_pt]; 
    ...
    for (int bin = 0 ; bin < nbin_jet_pt ; bin++)
    {
        ...
        hcorr_eec_nounf_l[bin]  = new TH1F(Form("hcorr_eec_nounf_l%i",bin) ,"", nbin_rl_nominal       ,rl_binning    );
        ...
        set_histogram_style(hcorr_eec_l[bin], corr_marker_color_jet_pt[bin], std_line_width, corr_marker_style_jet_pt[bin], std_marker_size+1);
        ...
        for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++)
        {
            ...
            hcorr_eec_nounf_l[bin]->Fill(R_L,purity*weight_pt/efficiency);
        }
        ...
        hcorr_eec_nounf_l[bin]->Scale(1./hcorr_jet[bin]->Integral());
        ...
        fout_linear->cd();
        hcorr_eec_l[bin]->Write();
        hcorr_eec_nounf_l[bin]->Write();
        gROOT->cd();
    }
    ...
    if (do_print) c->Print(Form("./plots/paircorreec_niter%i_linbinning_2dunf_ct.pdf",niter));
    ...
    if (do_print) c->Print(Form("./plots/paircorrtau_niter%i_2dunf_ct.pdf",niter));
    ...
    if (do_print) c->Print(Form("./plots/paircorreec_niter%i_2dunf_ct.pdf",niter));
    ...
    if (compare_to_nominal)
    {
        ...
        gPad->SetLogx(1);
        gPad->SetLogy(1);
        if (do_print) c->Print(Form("./plots/corrpseudata_data_comp_niter%i_ratio.pdf",niter));
    }
    ...
    if (compare_to_truth)
    {
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++)
        {
            ...
            ntuple_mc->Project(Form("htruth%i",bin),"R_L",eec_jet_pt_cut[bin]);

            // Normalize both to unity such that we can compare the shapes
            hcorr_eec_nonorm[bin]->Scale(1./hcorr_eec_nonorm[bin]->Integral(),"width");
            htruth[bin]->Scale(1./htruth[bin]->Integral(),"width");

            hcorr_eec_nonorm[bin]->Divide(htruth[bin]);
            for (int bin_rl = 1 ; bin_rl <= hcorr_eec_nonorm[bin]->GetNbinsX() ; bin_rl++)
            {
                hct_ratio->SetBinContent(bin_rl, bin + 1, hcorr_eec_nonorm[bin]->GetBinContent(bin_rl));
                hct_ratio->SetBinError(bin_rl, bin + 1, hcorr_eec_nonorm[bin]->GetBinError(bin_rl));
            } 
        }
        ...
        gPad->SetLogx(1);
        gPad->SetLogy(1);
        if (do_print) c->Print(Form("./plots/closuretest_niter%i_ratio.pdf",niter));
    }
}
