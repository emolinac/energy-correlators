...
void macro_print_fullcorreec_paircorr_2dunf_ct(int niter = nominal_niter, bool do_print = true, bool compare_to_nominal = false, bool compare_to_truth = true)
{
        ...
        if (do_print) 
                c->Print(Form("./plots/unfolded2d_niter%i_ratio_ct.pdf",niter));
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                for (int entry = 0 ; entry < ntuple_data->GetEntries() ; entry++) {
                        ...
                        hcorr_eec_nonorm[bin]->Fill(R_L,event_weight*purity*unfolding_weight*weight_pt/efficiency);
        ...
        if (do_print) 
                c->Print(Form("./plots/paircorrtau_niter%i_2dunf_ct.pdf",niter));
        ...
        if (do_print) 
                c->Print(Form("./plots/paircorreec_niter%i_2dunf_ct.pdf",niter));
        ...
        if (compare_to_nominal) {
                ...
                if (do_print) 
                        c->Print(Form("./plots/corrpseudata_data_comp_niter%i_ratio.pdf",niter));
        }
        ...
        if (compare_to_truth) {
                ...
                if (do_print) 
                c->Print(Form("./plots/closuretest_niter%i_ratio.pdf",niter));
        }
}
