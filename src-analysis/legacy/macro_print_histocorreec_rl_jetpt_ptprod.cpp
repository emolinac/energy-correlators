...
void macro_print_histocorreec_rl_jetpt_ptprod(int niter = nominal_niter, bool do_print = true)
{
        // Open the necessary files
        TFile* fout = new TFile((output_folder + Form("histos_eec_3dcorr_rl_jetpt_ptprod_niter%i.root",niter)).c_str(),"RECREATE");
        gROOT->cd();

        TFile* fcorr = new TFile((output_folder + namef_3dpaircorr_rl_jetpt_ptprod_histos).c_str()); 
        if (fcorr->IsZombie()) 
                return;
