...
void macro_print_fullcorreec_paircorr_mc_comp(int niter = 4, bool do_print = true, bool apply_weight_corr = false)
{
        ...
        float R_L, jet_pt, weight_pt, efficiency, purity, efficiency_factorized, purity_factorized, event_weight;
        ...
        if (apply_weight_corr) {
                TFile* fweightcorr = new TFile((output_folder + namef_histos_weight_corr).c_str());

                for(int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                        hweight_corr[bin] = (TH1F*) fweightcorr->Get(Form("h_weightcorr_%i",bin));

                        hcorr_eec[bin]->Multiply(hweight_corr[bin]);
                        hcorr_eec_syst[bin]->Multiply(hweight_corr[bin]);
                }
        }
        ...
        for(int bin = 0 ; bin < nbin_jet_pt ; bin ++) {
                ...
                s_data[bin]->Add(hcorr_eec_syst[bin],"E");
        ...
        c->Print(Form("./plots/data2mc_correec_unf-niter%i_weightcorr-%s.pdf",niter,(apply_weight_corr)?"yes":"no"));
        ...
        for(int bin = 0 ; bin < nbin_jet_pt ; bin ++) {
                ...
                hcorr_eec_syst[bin]->Divide(hmc[bin]);
        ...
        c_ratio->Print(Form("./plots/data2mcratio_correec_unf-niter%i_weightcorr-%s.pdf",niter,(apply_weight_corr)?"yes":"no"));
}