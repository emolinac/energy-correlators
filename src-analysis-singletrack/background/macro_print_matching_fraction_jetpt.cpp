...
void macro_print_matching_fraction_jetpt()
{
    ...
    l2->AddEntry(hratio_matched[0],Form("%.1f<Jet P_{T}(GeV)<%.1f",jet_pt_binning[0],jet_pt_binning[1]),"p");
    l2->AddEntry(hratio_matched[1],Form("%.1f<Jet P_{T}(GeV)<%.1f",jet_pt_binning[1],jet_pt_binning[2]),"p");
    l2->AddEntry(hratio_matched[2],Form("%.1f<Jet P_{T}(GeV)<%.1f",jet_pt_binning[2],jet_pt_binning[3]),"p");
    l2->Draw("SAME");

    tex->DrawLatexNDC(0.3,0.3,"simulations");

    c->Print(Form("../../plots/purity/matched_unmatched_jetpt_npair_deltarleq%.3f.pdf",R_L_res));    
}