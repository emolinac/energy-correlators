...
void macro_print_mc_at_e2c()
{
    ...
    for(int bin = 0 ; bin < Nbin_z_pt ; bin++)
    {
        ...
        // hmcreco_jet[bin] = new TH1F(Form("hmcreco_jet[%i]" ,bin),"",1,z_pt_binning[bin],z_pt_binning[bin+1]); 
        // hmcreco[bin]     = new TH1F(Form("hmcreco[%i]",bin),"",Nbin_R_L,rl_binning);
        ...
        // ntuple_mcreco->Project(Form("hmcreco[%i]",bin),"R_L",e2c_zpt_cut_weightpt[bin]);
        ntuple_mc->Project(Form("hmc[%i]" ,bin),"R_L",e2c_zpt_cut_weightpt[bin]);
        
        // ntuple_mcreco_jet->Project(Form("hmcreco_jet[%i]" ,bin),"jet_pt",pair_zpt_cut[bin]);
