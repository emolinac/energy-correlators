...
void macro_print_jes_chisquare(const int nbin = 50, double ptratio_min = 0.4 , double ptratio_max = 1.6, bool do_print = true)
{
        ...
        for (int bin = 0 ; bin < nbin_jet_pt ; bin++) {
                ...
                // Undo the JEC
                ntuple_jes_data->Project(Form("hdata_nojec[%i]",bin),"(jet_pt/z_pt)/jet_jec_cor",pair_jet_pt_cut[bin]);
                ntuple_jes_reco->Project(Form("hreco_nojec[%i]",bin),"(jet_pt/z_pt)/jet_jec_cor",pair_jet_pt_cut[bin]);
        ...
        if (do_print) 
                c->Print(Form("./plots/jes_beta_balance.pdf"));
        ...
        if (do_print) 
                c->Print(Form("./plots/jes_beta_chisquare.pdf"));
}